...
<div style="background: #fff;padding: 20px;">
    <div class="dataTable">
        ...
        <?php if (Yii::$app->user->can('role/power/create-role')):?>
        <button class="layui-btn layui-bg-blue" id="created">创建角色</button>
        <?php endif?>
    </div>
    <table class="layui-table" lay-data="{
            url:'/role/power/role-data'
            ,id:'test'
            ,page:true
            ,limits:[10,20,50,100]
            ,limit:20
        ...
        <tr>
            ...
            <th lay-data="{field:'right',width:240,toolbar: '#barDemo',fixed: 'right'}">操作</th>
        </tr>
        </thead>
    </table>
</div>
<script type="text/html" id="barDemo">
    ...
    <?php endif?>
    <?php if (Yii::$app->user->can('role/power/role-powers')):?>
        <a class="layui-btn layui-btn-xs layui-bg-cyan" lay-event="edit-power">权限编辑</a>
    <?php endif?>
</script>
<script>
    layui.use('table', function(){
        ...
        table.on('tool(test)', function(obj){
            ...
            if (layEvent === 'edit-power'){
                var url = '/role/power/role-powers?id='+data.name;
                layer.open({
                    type: 2,
                    title: '信息',
                    shadeClose: true,
        ...
        $('#created').on('click', function(){
            var url = '/role/power/create-role';
            layer.open({
                type: 2,
                title: '创建权限信息',
                shadeClose: true,
